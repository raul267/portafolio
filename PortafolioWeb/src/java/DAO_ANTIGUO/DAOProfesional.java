...
public class DAOProfesional implements CRUD<Profesional>
{
    ...
    private static String sql_update ="update profesional  set nombre = ?,direccion =? where id_profesional =?";
     ...
     private static String sql_selectIdByIdUser = "select * from profesional left join usuario using(id_usuario)  where  id_usuario=? ";
    ...
    public int ProfesionalIDByIdUser(int id) {
        
         try {
             Profesional p = null;
             PreparedStatement ps ;
             ps = objConn.getConection().prepareStatement(sql_selectIdByIdUser);
             ps.setInt(1, id);
             rs= ps.executeQuery();
             while (rs.next()) {                 
                 p = new Profesional(rs.getInt("id_profesional"),rs.getString("nombre"),
